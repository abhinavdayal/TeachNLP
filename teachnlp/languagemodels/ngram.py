...
class N_gram:
    ...
    def fit(self, training_corpus):
        """
            ...
            Sliding window method is used for counting frequency of n-grams.
        """
        ...
        for sentence in training_corpus:
            ...
            while(ptr2 < len(full_sentence)):
                ...
                nth_words_freqt_dict[full_sentence[ptr2]] = nth_words_freqt_dict.get(
                    full_sentence[ptr2], 0)+1
        ...
        for n_gram in n_gram_dict:
            top_words = sorted(
                n_gram_dict[n_gram], key=lambda x: n_gram_dict[n_gram][x])
    ...
    def strip_to_n(self, list_of_words):
        ...
        """
        return list_of_words[::-1][:self.n_gram-1][::-1]

    def predict_the_next_word(self, beginning_words=[]):
        """
            this method takes previous n-1 words and predicts the next word for n-gram model.
            If length of the given words list is not equal to n-1 then Padding with <PAD> token is done.
        """
        beginning_words = self.strip_to_n(beginning_words)
        beginning_words = self.pad_list(beginning_words)
    ...
    def generate_sentence(self, beginning_words=[]):
        ...
        window = self.strip_to_n(window)
        #print(window)
        window = self.pad_list(window)
